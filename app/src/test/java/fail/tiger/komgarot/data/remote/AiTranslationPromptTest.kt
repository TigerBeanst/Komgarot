...
class AiTranslationPromptTest {
    ...
    fun systemPromptDescribesLocalRegionCropTranslationOnly() {
        ...
        assertTrue(prompt.contains("layoutHints as a compactness target"))
        assertTrue(prompt.contains("Translate every readable source phrase completely"))
        assertTrue(prompt.contains("without abbreviating, summarizing, omitting content, or adding ellipses"))
    ...
    fun userPromptIncludesGlossaryForConsistentTerminology() {
        val prompt = aiTranslationUserPrompt(
            ...
            customInstructions = "",
            glossary = listOf(
                AiGlossaryEntry(source = "术语甲", target = "术语乙", note = "术语备注")
            )
        )

        assertTrue(prompt.contains("Glossary:"))
        assertTrue(prompt.contains("术语甲 => 术语乙 (术语备注)"))
